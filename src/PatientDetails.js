...
const PatientDetails = ({ patientId, onBackToList }) => {
  ...
  const [resourcePagination, setResourcePagination] = useState({
    observations: { page: 1, per_page: CONFIG.ui.defaultPageSize, total: 0, has_next: false, has_prev: false },
    diagnosticReports: { page: 1, per_page: CONFIG.ui.defaultPageSize, total: 0, has_next: false, has_prev: false },
    documentReferences: { page: 1, per_page: CONFIG.ui.defaultPageSize, total: 0, has_next: false, has_prev: false }
  });
  ...
  const loadTabData = async (tabName) => {
    ...
    try {
      ...
      switch (tabName) {
        case 'measurements':
          if (!loadedTabs.observations) {
            ...
            const obsResponse = await api.getPatientResources(patientId, 'Observation', CONFIG.ui.defaultPageSize, 1, 0);
            ...
            const transformedObs = (obsResponse.data || []).map(obs => {
              ...
              const extractValue = (obs) => {
                ...
                if (obs.valueBoolean !== undefined) return obs.valueBoolean ? 'True' : 'False';
              ...
              const extractUnit = (obs) => {
                ...
                if (obs.component?.length > 0) {
                  for (const comp of obs.component) {
                    if (comp.valueQuantity?.unit) return comp.valueQuantity.unit;
                  }
                }
                return '';
              };
              ...
              const extractDate = (obs) => {
                return obs.effectiveDateTime || obs.effectivePeriod?.start || obs.issued || obs.meta?.lastUpdated;
              };
            ...
            setLoadedTabs(prev => ({ ...prev, observations: true, measurements: true }));
          }
          break;
        ...
        case 'labs':
          if (!loadedTabs.observations) {
            ...
            const transformedObs = (obsResponse.data || []).map(obs => {
              return {
                ...obs,
                code_display: obs.code?.text || obs.code?.coding?.[0]?.display || obs.code?.display || 'Unknown',
                effective_date: obs.effectiveDateTime || obs.effectivePeriod?.start || obs.issued || obs.meta?.lastUpdated,
                value_quantity: obs.valueQuantity?.value || obs.valueString || (obs.valueBoolean !== undefined ? (obs.valueBoolean ? 'True' : 'False') : null),
            ...
            setLoadedTabs(prev => ({ ...prev, observations: true, measurements: true, labs: true }));
          }
          break;
        ...
        case 'notes':
          ...
          if (!loadedTabs.diagnosticReports) {
            promises.push(api.getPatientResources(patientId, 'DiagnosticReport', CONFIG.ui.defaultPageSize, 1, 0));
          }
          if (!loadedTabs.documentReferences) {
            promises.push(api.getPatientResources(patientId, 'DocumentReference', CONFIG.ui.defaultPageSize, 1, 0));
          }
          ...
          if (promises.length > 0) {
            ...
            if (!loadedTabs.diagnosticReports && results[0]) {
              ...
              const rawReports = results[0].status === 'fulfilled' && result.success ? 
                result.data || [] : [];
              // Transform DiagnosticReports for Notes component compatibility
              newData.diagnosticReports = rawReports.map(report => ({
                ...report,
                source_filename: `${report.resourceType || 'DiagnosticReport'}_${report.id || 'unknown'}.fhir`,
                date: report.effectiveDateTime || report.issued || report.meta?.lastUpdated
              }));
              
              // Update pagination state for DiagnosticReports
              if (results[0].status === 'fulfilled' && result.success && result.pagination) {
                setResourcePagination(prev => ({
                  ...prev,
            ...
            if (!loadedTabs.documentReferences) {
              ...
              if (results[docIndex]) {
                ...
                const rawDocs = results[docIndex].status === 'fulfilled' && result.success ? 
                  result.data || [] : [];
                // Transform DocumentReferences for Notes component compatibility
                newData.documentReferences = rawDocs.map(doc => ({
                  ...doc,
                  source_filename: `${doc.resourceType || 'DocumentReference'}_${doc.id || 'unknown'}.fhir`,
                  date: doc.date || doc.effectiveDateTime || doc.issued || doc.meta?.lastUpdated
                }));
                
                // Update pagination state for DocumentReferences
                if (results[docIndex].status === 'fulfilled' && result.success && result.pagination) {
                  setResourcePagination(prev => ({
                    ...prev,
            ...
            setLoadedTabs(prev => ({ ...prev, diagnosticReports: true, documentReferences: true, notes: true }));
          }
          break;
        ...
        case 'general':
          ...
          if (!loadedTabs.general) {
            console.log('General Information tab loaded - Personal Information ready, Overview loads on demand');
      ...
      setOriginalData(prev => ({
        ...
        documentReferences: newData.documentReferences || prev.documentReferences,
  ...
  const handleResourcePageChange = async (resourceType, newPage) => {
    try {
      ...
      const response = await api.getPatientResources(patientId, resourceType, CONFIG.ui.defaultPageSize, newPage, offset);
      ...
      if (response.success) {
        ...
        if (response.pagination) {
          setResourcePagination(prev => ({
            ...prev,
            [resourceType === 'Observation' ? 'observations' : 
             resourceType === 'DiagnosticReport' ? 'diagnosticReports' : 'documentReferences']: response.pagination
          }));
        }
      }
    } catch (error) {
      console.error(`Error loading page ${newPage} for ${resourceType}:`, error);
    } finally {
  ...
  const loadAvailableResources = async () => {
    try {
      ...
      if (metadataResponse.success && metadataResponse.supported_resources) {
        ...
        setAvailableResources(resources);
        console.log('📋 Available resources loaded via fallback:', resources.length);
      }
    } catch (error) {
  ...
  const loadTabDataInBackground = async (tabId, resourceType) => {
    try {
      ...
      const schema = schemaResponse.success ? schemaResponse.inferred_schema?.full_column_list || [] : [];
      ...
      const transformedData = resourceData.map(resource => {
        ...
        return {
          ...
          date: resource.date || resource.effectiveDateTime || resource.authoredOn || 
                resource.issued || resource.recorded || resource.performedDateTime || 'N/A',
          display: resource.code?.text || resource.code?.display || 
                  resource.name || resource.title || resource.type?.text || 'N/A',
          patient_reference: resource.subject?.reference || resource.patient?.reference || 'N/A'
        };
      });

      // Update the tab with loaded data
      setDynamicTabs(prev => prev.map(tab => 
        tab.id === tabId 
          ? {
              ...tab,
      ...
      console.log(`Dynamic tab data loaded: ${resourceType} (${transformedData.length} items)`);
  ...
  const getResourceDataForTab = async (resourceType) => {
    ...
    if (medicalDataKey && medicalData[medicalDataKey] && medicalData[medicalDataKey].length > 0) {
      console.log(`Reusing existing medical data for ${resourceType}`);
      return medicalData[medicalDataKey];
    } else if (allResourceData[resourceType] && allResourceData[resourceType].length > 0) {
      ...
      const response = await api.getPatientResources(patientId, resourceType, 100);

      if (response.success) {
        const resourceData = response.data || [];
        // Cache for future use
        setAllResourceData(prev => ({ ...prev, [resourceType]: resourceData }));
        return resourceData;
      } else {
  ...
  const categorizeObservation = (obs) => {
    ...
    if (obs.category && obs.category.length > 0) {
      ...
      if (category.coding && category.coding.length > 0) {
        return category.coding[0].display || category.coding[0].code || 'Observation';
  ...
  const applyFiltersAndSorting = () => {
    ...
    const hasData = originalData.observations.length > 0 || 
                   originalData.diagnosticReports.length > 0 || 
                   originalData.documentReferences.length > 0 ||
                   Object.values(originalData.medicalData || {}).some(arr => Array.isArray(arr) && arr.length > 0);
    ...
    if (tabLoading[activeTab]) {
      return (
        <div style={{ 
          ...
          <h3>Loading {activeTab === 'general' ? 'General Information' : activeTab === 'measurements' ? 'Measurements' : activeTab === 'labs' ? 'Laboratory Data' : 'Notes'} Data</h3>
    ...
    switch (activeTab) {
      case 'measurements':
        return <Measurements 
          ...
          onPageChange={(newPage) => handleResourcePageChange('Observation', newPage)}
          loading={tabLoading.measurements}
          {...commonProps} 
        />;
      case 'labs':
        return <LabsContainer 
          observations={observations} 
      ...
      case 'notes':
        return <Notes 
          ...
          onPageChange={(resourceType, newPage) => {
            const fhirResourceType = resourceType === 'documentReferences' ? 'DocumentReference' : 'DiagnosticReport';
            handleResourcePageChange(fhirResourceType, newPage);
          }}
          loading={tabLoading.notes}
        />;
      default:
        return <GeneralInformation patientData={patientData} patientId={patientId} {...commonProps} />;
    }
  };
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f5f5f5' }}>
        <div style={{ textAlign: 'center', padding: '40px' }}>
          ...
          <button onClick={() => loadBasicPatientData()} style={{ 
            padding: '12px 24px', 
            background: '#007bff', 
            color: 'white', 
            border: 'none', 
