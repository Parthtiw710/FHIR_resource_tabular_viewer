...
import { initializeFilterCache, getFilterResources, getFilterCacheStats, clearFilterCache } from './filterResourceCache';
import './App.css';
...
const MainPage = () => {
  ...
  const validateRestoredFilters = (filters) => {
    ...
    Object.keys(cleanedFilters).forEach(key => {
      ...
      // Remove empty arrays
      else if (Array.isArray(cleanedFilters[key]) && cleanedFilters[key].length === 0) {
        delete cleanedFilters[key];
      }
      // Remove empty objects (except age_range which can have min_age or max_age)
      else if (typeof cleanedFilters[key] === 'object' && !Array.isArray(cleanedFilters[key]) && 
               key !== 'age_range' && Object.keys(cleanedFilters[key]).length === 0) {
        delete cleanedFilters[key];
      }
    });
  ...
  useEffect(() => {
    ...
    if (restored) {
      ...
      setTimeout(() => {
        ...
        const hasFilters = parsedState?.filters && Object.keys(parsedState.filters).length > 0;
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      const hasActiveFilters = (activeFilters.filters && Object.keys(activeFilters.filters).length > 0) || 
                                (activeFilters.general_filters && Object.keys(activeFilters.general_filters).length > 0);
      ...
      if (hasActiveFilters) {
        ...
        console.log(`🔍 Main effect triggered - isPageChanging: ${isPageChanging}, page: ${pagination.page}`);
        if (isPageChanging) {
          console.log('⏭️ Skipping main effect - page change in progress');
          return;
        }
        console.log('🔍 Loading patients with filters from main effect:', activeFilters);
        loadPatientsWithFilters(activeFilters, pagination.page);
        return;
      }
  ...
  }, [pagination.page, searchTerm, viewMode, pagination.per_page, activeFilters, isPageChanging]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedState) {
        ...
        if (Object.keys(state.filters || {}).length > Object.keys(validatedFilters).length) {
  ...
  const loadPatients = async (page = 1) => {
    ...
    try {
      ...
      if (JSON.stringify(validatedFilters) !== JSON.stringify(activeFilters)) {
        console.log('🧹 Active filters were cleaned during loading:', validatedFilters);
      ...
      console.log('🔍 Loading patients with params:', JSON.stringify(params, null, 2));
      ...
      if (response.success) {
        ...
        console.log('State updated - patients set to:', transformedPatients.length);
        ...
        console.log('Patients loaded:', transformedPatients.length, `(page ${page})`);
  ...
  const handleFilterChange = (filterPayload) => {
    ...
    const hasResourceFilters = filterPayload.filters && Object.keys(filterPayload.filters).length > 0;
    const hasGeneralFilters = filterPayload.general_filters && Object.keys(filterPayload.general_filters).length > 0;
  ...
  const loadPatientsWithFilters = async (filters, page = 1) => {
    try {
      ...
      const response = await api.loadPatientsWithFilters(params, appliedFilters);
      ...
      if (response.success && response.data) {
        ...
        const isOptimized = response.pagination.optimized || response.query_optimized;
        ...
        if (response.pagination.loaded_all_filtered || returnedAllResults) {
          ...
          console.log(`📄 All filtered records loaded (${transformedPatients.length}) - switching to client-side pagination`);
          setPagination(prev => ({
            ...prev,
            page: prev.page, // Keep the current page from state, not the requested page
            total: transformedPatients.length,
            has_next: Math.ceil(transformedPatients.length / prev.per_page) > prev.page,
          ...
          setPagination(prev => ({
            ...prev,
            page: page,
            total: response.pagination.total || response.matching_patient_count || transformedPatients.length,
            has_next: response.pagination.has_next || false,
            has_prev: page > 1,
        ...
        console.log(`✅ Loaded ${transformedPatients.length} patients from ${response.matching_patient_count} total matches`);
        if (response.query_optimized) {
          console.log(`⚡ Query was optimized: effective page size = ${response.effective_page_size}`);
        }
      } else {
  ...
  const extractAppliedFiltersForMetadata = (filterPayload) => {
    ...
    if (filterPayload.filters && typeof filterPayload.filters === 'object') {
      Object.entries(filterPayload.filters).forEach(([key, filterData]) => {
        ...
        if (key === 'age_range' && filterData && typeof filterData === 'object' && 
    ...
    if (filterPayload.general_filters && typeof filterPayload.general_filters === 'object') {
      Object.entries(filterPayload.general_filters).forEach(([key, filterData]) => {
        // Handle age range filter (object with min_age/max_age)
        if (key === 'age_range' && filterData && typeof filterData === 'object' && 
            (filterData.min_age !== undefined || filterData.max_age !== undefined)) {
          appliedFilters[key] = filterData;
  ...
  const handlePageChange = (newPage) => {
    if (newPage !== pagination.page && newPage > 0) {
      console.log(`📄 Navigating to page ${newPage} (isPageChanging: ${isPageChanging})`);
      ...
      if (pagination.loaded_all_filtered) {
        console.log('📄 Using client-side pagination for all loaded filtered records');
        setIsPageChanging(false);
      ...
      const hasResourceFilters = activeFilters.filters && Object.keys(activeFilters.filters).length > 0;
      const hasGeneralFilters = activeFilters.general_filters && Object.keys(activeFilters.general_filters).length > 0;
      const hasActiveFilters = hasResourceFilters || hasGeneralFilters;
  ...
  const handleFhirSearchResults = async (searchResults, searchParams) => {
    ...
    if (conditionCode) {
      ...
      try {
        ...
        const conditionSearchResponse = await api.searchPatientsByCondition(conditionCode, {
          per_page: CONFIG.ui.defaultPageSize,
          page: 1
        });
        
        console.log('🔍 Backend condition search response:', conditionSearchResponse);
        
        if (conditionSearchResponse.success && conditionSearchResponse.data && conditionSearchResponse.data.length > 0) {
          console.log(`🔍 Found ${conditionSearchResponse.data.length} patients with condition ${conditionCode} via backend`);
          ...
          console.warn('🔍 Backend condition search found no patients, trying direct FHIR results');
        }
      } catch (error) {
        console.warn('🔍 Condition search API failed, falling back to FHIR results:', error);
      }
    }
    ...
    if (searchResults && searchResults.entry && searchResults.entry.length > 0) {
      // Extract patient IDs from FHIR search results
      const patientIds = searchResults.entry.map((entry, index) => {
      ...
      console.log(`🔍 Extracted ${patientIds.length} patient IDs from FHIR results:`, patientIds);
        ...
        try {
          ...
          if (patientsResponse.success && patientsResponse.data) {
            ...
            const filteredPatients = patientsResponse.data.filter(patient => {
              ...
              console.log(`🔍 Patient ${patient.id} matches FHIR results: ${matches}`);
              return matches;
            });
            
            console.log(`🔍 Successfully filtered to ${filteredPatients.length} matching patients from FHIR search`);
            ...
            if (filteredPatients.length > 0) {
              ...
              console.warn('🔍 No patients from backend matched FHIR search results');
              setPatients([]);
              setOriginalPatients([]);
              setPagination(prev => ({ ...prev, total: 0 }));
            }
          } else {
            console.error('🔍 Backend failed to load patients:', patientsResponse);
            setError('Failed to load patient data from backend');
          }
        } catch (error) {
          console.error('🔍 Error loading FHIR search results:', error);
          setError(`Failed to load patients matching condition: ${error.message}`);
        }
      } else {
...
function PatientDetailsWrapper() {
  ...
  return <PatientDetails patientId={patientId} onBackToList={handleBackToList} />;
}
